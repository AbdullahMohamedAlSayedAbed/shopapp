    ...
    return BlocConsumer<GetProfileCubit, GetProfileState>(
      listener: (context, state) {
        ...
        if (state is UpdateProfileSuccess) {
          ...
          showTost(message: state.updateProfileModel.message.toString(), state: ToastStates.SUCCESS );
        }
        if (state is UpdateProfileFailure) {
          showTost(message: state.errMessage.toString(), state: ToastStates.ERROR);
        }
      },
      builder: (context, state) {
        if (state is GetProfileSuccess || state is UpdateProfileSuccess ||
